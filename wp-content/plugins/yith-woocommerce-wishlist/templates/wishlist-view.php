...
<div class="container">
    <form id="yith-wcwl-form" action="<?php echo esc_url(YITH_WCWL()->get_wishlist_url('view' . ( $wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : '' ))) ?>" method="post" class="woocommerce">
        ...
        if (!empty($page_title)) :
            ?>
            <div style="text-align: center;font-family: sans-serif;" class="wishlist-title <?php echo ( $wishlist_meta['is_default'] != 1 && $is_user_owner ) ? 'wishlist-title-with-form' : '' ?>">
        ...
        <!-- WISHLIST TABLE -->
        <table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr($pagination) ?>" data-per-page="<?php echo esc_attr($per_page) ?>" data-page="<?php echo esc_attr($current_page) ?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr($wishlist_meta['ID']) : '' ?>" data-token="<?php echo (!empty($wishlist_meta['wishlist_token']) && is_user_logged_in() ) ? esc_attr($wishlist_meta['wishlist_token']) : '' ?>" border="0" width="100%" style="text-align:center;margin:50px 0px 70px 0px">

            <?php $column_count = 2; ?>

            <thead style="height: 45px; background: #1e72d4;color: white">
                <tr>
                    <?php if ($show_cb) : ?>
                    ...
                    <th class="product-thumbnail" style="text-align: center;"><span class="nobr">Product Image</span></th>

                    <th class="product-name" style="text-align: center;">
                    ...
                    <?php if ($show_stock_status) : ?>
                        ...
                        <th class="product-stock-stauts" style="text-align: center;">
                            <span class="nobr">
                    ...
                    foreach ($wishlist_items as $item) :
                        ...
                        if ($product !== false && $product->exists()) :
                            ...
                            <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                                <?php if ($show_cb) : ?>
                                    <td class="product-checkbox" style="border:1px solid #ccc;">
                                ...
                                <?php if ($is_user_owner): ?>
                                    <td class="product-remove" style="border:1px solid #ccc;">
                                        <div>
                                            <a href="<?php echo esc_url(add_query_arg('remove_from_wishlist', $item['prod_id'])) ?>" class="remove remove_from_wishlist" title="<?php _e('Remove this product', 'yith-woocommerce-wishlist') ?>"><img src="<?php echo esc_url( YITH_WCWL_URL . 'assets/images/cross1.png' ) ?>" class="ajax-loading" alt="loading" width="20" height="20"/></a>
                                        </div>
                                    </td>
                                <?php endif; ?>

                                <td class="product-thumbnail" style="border:1px solid #ccc;">
                                    <a href="<?php echo esc_url(get_permalink(apply_filters('woocommerce_in_cart_product', $item['prod_id']))) ?>">
                                        <?php echo $product->get_image() ?>
                                    </a>
                                </td>

                                <td class="product-name" style="border:1px solid #ccc;">
                                ...
                                <?php if ($show_price) : ?>
                                    <td class="product-price" style="border:1px solid #ccc;">
                                        <?php
                                ...
                                <?php if ($show_stock_status) : ?>
                                    <td class="product-stock-status" style="border:1px solid #ccc;">
                                        <?php
                                ...
                                <?php if ($show_last_column): ?>
                                    <td class="product-add-to-cart" style="border:1px solid #ccc;">
                                        <!-- Date added -->
                                        <?php
